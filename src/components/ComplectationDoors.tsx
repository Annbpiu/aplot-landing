...
export const ComplectationDoors = () => {
    ...
    return (
        <div id="DoorComplectation"
             className="h-full flex flex-col justify-between min-h-[100svh] w-full bg-white p-8 lg:px-30">
            {/* Заголовок */}
            ...
            <div
                ...
                {doorOptions
                    ...
                    .map((door) => (
                        <div
                            key={door.name}
                            className="flex justify-end md:justify-center h-120 items-center cursor-pointer"
                            onClick={() => setSelectedDoor(door.name)}
                        >
                            <img
                                src={door.img}
                                alt={door.name}
                                className={`xl:w-40 xl:h-80 lg:w-50 absolute lg:relative lg:h-50 md:w-50 md:h-80 h-75 w-50 object-contain transition-all xl:duration-300 mx-1 ${
